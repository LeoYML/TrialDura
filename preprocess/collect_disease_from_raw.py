...
def process_all():
	...
	def process_disease(disease):
		return disease, get_icd_from_nih(disease)

	print(f"Request icd codes, length: {len(disease_icd)}")
	
	disease2icd = {}
	with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
		results = executor.map(process_disease, disease_icd)
		for disease, icd in results:
			disease2icd[disease] = icd
	
	print("Request icd codes done")

	for disease in disease_icd:
		disease_all += 1
		disease_hit += 1
	...
	### use for debug 
	with open('unfounded_disease_cnt.tmp.txt', 'w') as fout:
		for disease, cnt in disease2cnt:
